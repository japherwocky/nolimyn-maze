...
// Hopefully this gives PCs a boost to parry v mobs?
	if (IS_NPC(ch)) 
		chance *=2;
	...
	// fwah?  are we even using posture?
	/* posture */
	if (!IS_NPC(ch)){
		if (check_posture(ch) == POSTURE_DEFENSE && style_check(gsn_posture, ch->pcdata->style))
			chance += 20;
...
//	does this do what I think it does?  batter is OP, and we check it latter on
	chance -= ch->batter;
...
// what are we actually checking?  to learn, or?
...
/*
 * I wanna cleave like everyone else!
