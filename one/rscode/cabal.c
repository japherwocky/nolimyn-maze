	...
	if(ch->cabal)
	{
		for(i=0; cabal_skills[i].skill; i++)
		{
			...
			if(ch->pcdata->learned[skill_lookup(cabal_skills[i].skill)] < 2)
				ch->pcdata->learned[skill_lookup(cabal_skills[i].skill)] = 90;
		}
	}
	else
	{
...
{
	if(cabal_down(ch,CABAL_HORDE)) return;
	AFFECT_DATA af;
	OBJ_DATA *obj;
	char buf[MSL];
	if(IS_NPC(ch))
...
{
	if(cabal_down(ch,CABAL_BOUNTY)) return;
...
{
	if(cabal_down(ch,CABAL_SCION)) return;
	CHAR_DATA *victim = (CHAR_DATA *) vo;
	int mod = (ch->level * 2);
...
void spell_scribe( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	*/
	...
	obj->level = 13 + (level) + (ipower / 200);
	obj->item_type = ITEM_SCROLL;
	obj->value[0] = 4 + (level) + (ipower / 100);
	obj->value[1] = skill;
	obj->value[2] = -1;
	obj->value[3] = -1;
	obj->value[4] = -1;
...
void spell_piety(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
	...
	if(cabal_down(ch,CABAL_PHALANX)) return;
